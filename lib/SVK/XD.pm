...
sub do_propset {
    ...
    my ($txn, $xdroot);
    my $entry = $info->{checkout}->get ($arg{copath});
    $entry->{'.schedule'} ||= '';
    $entry->{'.newprop'} ||= {};
...
sub _delta_file {
    ...
    my $cinfo = $arg{cinfo} || $info->{checkout}->get ($arg{copath});
...
sub _delta_dir {
	...
	next if m/$ignore/;
	my $ccinfo = $info->{checkout}->get ("$arg{copath}/$_");
...
sub resolved_entry {
    my ($info, $entry) = @_;
    my $val = $info->{checkout}->get ($entry);
...
sub get_props {
    ...
    $entry = $info->{checkout}->get ($copath) if $copath;
    $entry->{'.newprop'} ||= {};
    $entry->{'.schedule'} ||= '';
