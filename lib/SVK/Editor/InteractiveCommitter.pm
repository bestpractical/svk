...
sub close_edit {
    ...
    return $self->{storage}->close_edit($pool) if $self->{commit_phase};
    ...
    my $ui = SVK::Editor::InteractiveCommitter::UI->new($self);
    $ui->run;
...
sub abort_edit {
    ...
    $self->{storage}->abort_edit($pool) if $self->{commit_phase};
}
...
sub open_root {
    ...
    if ($self->{commit_phase}) {
        ...
        push @{$self->{actions}}, $self->{info}{''} =
            SVK::Editor::InteractiveCommitter::OpenDirectoryAction->
...
sub add_file {
    ...
    if ($self->{commit_phase}) {
        ...
        push @{$self->{actions}}, $self->{info}{$path} =
            SVK::Editor::InteractiveCommitter::AddFileAction->new($pdir, @_);
    }
...
sub open_file {
    ...
    if ($self->{commit_phase}) {
        ...
        push @{$self->{actions}}, $self->{info}{$path} =
            SVK::Editor::InteractiveCommitter::ModifyFileAction->new($pdir, @_);
    }
...
sub change_file_prop {
    ...
    if ($self->{commit_phase}) {
        my $action = $self->{info}{$path}{props}{$name};
        $action->on_change_file_prop_commit(@_) if $action->enabled;
    } else {
        push @{$self->{actions}}, $self->{info}{$path}{props}{$name} = 
            SVK::Editor::InteractiveCommitter::ModifyFilePropAction->new($path, @_);
    }
}
...
sub delete_entry {
    ...
    if ($self->{commit_phase}) {
        ...
        push @{$self->{actions}}, $self->{info}{$path} =
            SVK::Editor::InteractiveCommitter::DeleteFileAction->new($parent, @_);
    }
}
...
sub add_directory {
    ...
    if ($self->{commit_phase}) {
        ...
        push @{$self->{actions}}, $self->{info}{$path} =
            SVK::Editor::InteractiveCommitter::AddDirectoryAction->new($pdir, @_);
    }
...
sub open_directory {
    ...
    if ($self->{commit_phase}) {
        ...
        push @{$self->{actions}}, $self->{info}{$path} =
            SVK::Editor::InteractiveCommitter::OpenDirectoryAction->new($pdir, @_);
    }
...
sub change_dir_prop {
    ...
    if ($self->{commit_phase}) {
        ...
        $action->on_change_dir_prop_commit(@_) if $action->enabled;
    } else {
        push @{$self->{actions}}, $self->{info}{$path}{props}{$name} = 
            SVK::Editor::InteractiveCommitter::ModifyDirectoryPropAction->new($path, @_);
    }
}
...
package SVK::Editor::InteractiveCommitter::UI;
use SVK::I18N;
...
package SVK::Editor::InteractiveCommitter::Action;
...
package SVK::Editor::InteractiveCommitter::AddFileAction;
use base qw(SVK::Editor::InteractiveCommitter::Action);
...
package SVK::Editor::InteractiveCommitter::ModifyFileAction;
use base qw(SVK::Editor::InteractiveCommitter::Action);
...
sub on_apply_textdelta {
    ...
    my $fh1 = $editor->{cb_localmod}($path, '', $pool)->[0];
...
sub on_apply_textdelta_commit {
    ...
    my $handle = SVK::Editor::InteractiveCommitter::Action::on_apply_textdelta_commit(@_);
    ...
    if ($handle && $#{$handle} >= 0) {
        ...
        if ($editor->{send_fulltext}) {
            ...
            my $ofh = $editor->{cb_localmod}($path, '', $pool)->[0];
...
package SVK::Editor::InteractiveCommitter::ModifyBinaryFileAction;
use base qw(SVK::Editor::InteractiveCommitter::AddFileAction);
...
package SVK::Editor::InteractiveCommitter::DeleteFileAction;
use base qw(SVK::Editor::InteractiveCommitter::Action);
...
package SVK::Editor::InteractiveCommitter::ModifyFilePropAction;
use base qw(SVK::Editor::InteractiveCommitter::Action);
...
sub new {
    ...
    my ($l, $r) = $editor->{cb_localprop}($path, $name, $pool);
    ($l, $r) = map { !length || /\n$/ ? $_ : "$_\n"}
...
package SVK::Editor::InteractiveCommitter::AddDirectoryAction;
use base qw(SVK::Editor::InteractiveCommitter::Action);
...
package SVK::Editor::InteractiveCommitter::OpenDirectoryAction;
use base qw(SVK::Editor::InteractiveCommitter::Action);
...
package SVK::Editor::InteractiveCommitter::ModifyDirectoryPropAction;
use base qw(SVK::Editor::InteractiveCommitter::ModifyFilePropAction);
